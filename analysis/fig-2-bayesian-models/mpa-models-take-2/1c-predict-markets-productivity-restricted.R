...
markets.prod <- markets.prod %>% 
  mutate(Grav_Markets.max.raw = (Grav_Markets.max * 2*sd.var1) + mean.var1, 
         npp_mean.raw = (npp_mean * 2*sd.var2) + mean.var2)

summary(markets.prod$Grav_Markets.max.raw)
summary(markets.prod$npp_mean.raw)
summary(markets.prod$median)
...
summary(markets.prod$dist)
summary(markets.prod$median.colour)

gg.markets.prod.restricted <- ggplot(data = markets.prod, 
                          aes(x = Grav_Markets.max.raw, y = npp_mean.raw)) + 
  ...
  #                      breaks = c(0.025, 0.1, 0.15, 0.2),
  #                      labels = c("<10%", " 10%", " 15%", " 20%"),
