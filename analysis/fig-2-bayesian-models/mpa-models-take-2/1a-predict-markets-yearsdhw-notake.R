...
#what to do about no <10% values? 
#will need to change colour space
library(RColorBrewer)
spectral <- brewer.pal(6, name = "Spectral")

no.red.spectral <- palette(spectral[4:6])
no.red.spectral


gg.yearsdhw.markets.notake <- ggplot(data = yearsdhw.markets, aes(x = years.sinceDHW.raw, y = Grav_Markets.max.raw)) + 
  ...
  scale_fill_gradientn(colors = no.red.spectral) +
  scale_colour_gradientn(colors = no.red.spectral) +
  xlab("Years since max DHW") +
  ylab("Market gravity") + 
  theme(legend.position = "none") #+ 
  #coord_flip()
gg.yearsdhw.markets.notake
